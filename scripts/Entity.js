...
define(function(require, exports, module) {
  ...
  var GlMatrix = require('./GlMatrix');
    ...
    * }
    */
   Entity.create = function create(desc, e)
   {
    ...
    */
   Entity.createStyle = function createStyle(s)
   {
      var style = {
         color: [128,128,128],
         diffuse: 1.0,
      ...
      if (s) Util.combine(style, s);
      return style;
   };
   
   Util.extend(Entity, BaseEntity, {
       ...
       * 
       * @param fn {function}    onRender handler signature: function(ctx, x, y, w) this = Entity,
      ...
      generatePolygonNormals: function generatePolygonNormals()
      {
         if (this.polygons)
         {
            ...
            for (var i=0, vertices, x1, y1, z1, x2, y2, z2; i<polygons.length; i++)
            {
               ...
               polygons[i].normal = Util.calcNormalVector(x1, y1, z1, x2, y2, z2);
            }
         }
      },
      ...
      initCoordinateBuffers: function initCoordinateBuffers()
      {
         ...
         if (this._worldcoords === null || this._worldcoords.length < len)
         {
            ...
            for (var i=0; i<len; i++) this._worldcoords[i] = GlMatrix.vec4.create();
         }
         if (this._cameracoords === null || this._cameracoords.length < len)
         {
            this._cameracoords = new Array(len);
            for (var i=0; i<len; i++) this._cameracoords[i] = GlMatrix.vec4.create();
         }
         if (this._coords === null || this._coords.length < len)
         {
            this._coords = new Array(len);
            for (var i=0; i<len; i++) this._coords[i] = GlMatrix.vec4.create();
         }
         if (this._clip === null || this._clip.length < len)
         {
            this._clip = new Uint32Array(len);
         }
      },
    ...
    * }
    */
   Entity.debug = function debug(entity, config)
   {
      ...
      // set the config
      Util.combine(debugEntity.config, config);
