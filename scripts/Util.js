/**
 * @fileoverview phoria - Utilities and helpers.
...
define(function(require, exports, module) {
  ...
  */
  Util.extend = function extend(subc, superc, overrides)
  {
    var F = function() {}, i;
  ...
  */
  Util.augment = function augment(r, s)
  {
  ...
  */
  Util.merge = function merge(target, src)
  {
    ...
    else
    {
      ...
      Object.keys(src).forEach(function (key) {
        ...
        else
        {
          ...
          {
            dst[key] = Util.merge(target[key], src[key]);
          }
        }
      });
    }
  ...
  */
  Util.combine = function combine(target, src)
  {
    ...
    else
    {
      Object.keys(src).forEach(function (key) {
        ...
        else
        {
          ...
          Util.combine(target[key], src[key]);
        }
      });
    }
  };
  ...
  */
  Util.isIdentity = function isIdentity(mat)
  {
    return (
      mat[0] === 1 && 
      mat[1] === 0 &&
  ...
  */
  Util.calcNormalVector = function calcNormalVector(x1, y1, z1, x2, y2, z2)
  {
    ...
    return GlMatrix.vec3.normalize(v, v);
  }

  /**
  * Calculate the angle between two 3D vectors
  */
  Util.thetaTo = function thetaTo(v1, v2)
  {
    return Math.acos(GlMatrix.vec3.dot(v1, v2) / (Math.sqrt(v1[0] * v1[0] + v1[1] * v1[1] + v1[2] * v1[2]) * Math.sqrt(v2[0] * v2[0] + v2[1] * v2[1] + v2[2] * v2[2])));
  };

  /**
  * Return a vec3 representing the average world coordinate for the given polygon vertices
  */
  Util.averagePolyVertex = function averagePolyVertex(vertices, worldcoords)
  {
  ...
  */
  Util.averageObjectZ = function averageObjectZ(coords)
  {
    var av = 0;
  ...
  */
  Util.populateBuffer = function populateBuffer(len, fnFactory)
  {
  ...
  */
  Util.sortPolygons = function sortPolygons(polygons, worldcoords)
  {
  ...
  */
  Util.sortEdges = function sortEdges(edges, coords)
  {
  ...
  */
  Util.sortPoints = function sortPoints(coords, worldcoords)
  {
  ...
  */
  Util.generateTesselatedPlane = function generateTesselatedPlane(vsegs, hsegs, level, scale, generateUVs)
  {
  ...
  * @param scale   optional scaling factor
  */
  Util.generateUnitCube = function generateUnitCube(scale)
  {
    var s = scale || 1;
    return {
  ...
  * @param scale   optional scaling factor
  */
  Util.generatePyramid = function generatePyramid(scale)
  {
    var s = scale || 1;
    return {
  ...
  * @param scale   optional scaling factor
  */
  Util.generateIcosahedron = function generateIcosahedron(scale)
  {
  ...
  */
  Util.subdivide = function subdivide(v, p)
  {
    var vertices = [],
    polys = [];
  ...
  */
  Util.generateCylinder = function generateCylinder(radius, length, strips)
  {
  ...
  * }
  */
  Util.generateCuboid = function generateCuboid(desc)
  {
  ...
  */
  Util.generateSphere = function generateSphere(scale, lats, longs, generateUVs)
  {
  ...
  */
  Util.generateRadialGradientBitmap = function generateRadialGradientBitmap(size, innerColour, outerColour)
  {
  ...
  *    }
  */
  Util.request = function request(config)
  {
  ...
  *    }
  */
  Util.importGeometryWavefront = function importGeometryWavefront(config)
  {
  ...
  Util.calculatePolarFromPlanar = function calculatePolarFromPlanar(planar)
  {
    // array positions correspond to: r = [0], t = [1], p = [2]
    var point = new GlMatrix.vec3.create();
    // r is radius and equals the length of the planar vector
    point[0] = GlMatrix.vec3.length(planar);
  ...
  Util.calculatePlanarFromPolar = function calculatePlanarFromPolar(polar)
  {
    return new GlMatrix.vec3.fromValues(
  ...
  Util.planeLineIntersection = function planeLineIntersection(planeNormal, planePoint, lineVector, linePoint)
  {
    ...
    var dotProduct = GlMatrix.vec3.dot(lineVector, planeNormal);
    // check that click vector is not parallel to polygon
    if (dotProduct !== 0)
    {
      var pointVector = new GlMatrix.vec3.create();
      GlMatrix.GlMatrix.vec3.subtract(pointVector, planePoint, linePoint);
      var lineScalar = GlMatrix.vec3.dot(planeNormal, pointVector) / dotProduct;
      var intersection = GlMatrix.vec3.create();
      GlMatrix.GlMatrix.vec3.scaleAndAdd(intersection, linePoint, lineVector, lineScalar);
      return intersection;
    }
    else
    {
  ...
  Util.intersectionInsidePolygon = function intersectionInsidePolygon(polygon, points, intersection)
  {
    // get absolute values of polygons normal vector
    var absNormal = GlMatrix.vec3.fromValues(Math.abs(polygon._worldnormal[0]), Math.abs(polygon._worldnormal[1]), Math.abs(polygon._worldnormal[2]));
    ...
    for (var l = 0; l < polygon.vertices.length; l++)
    {
      ...
      if (Util.sectionLineIntersect2D(point1, point2, intersection2D, testVector))
      {
  ...
  Util.sectionLineIntersect2D = function sectionLineIntersect2D(p1, p2, p, v)
  {
    ...
    var svCross = GlMatrix.vec3.create();
