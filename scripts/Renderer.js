      ...
      calcNormalBrightness: function calcNormalBrightness(position, normal, scene, obj)
      {
         ...
         for (var e=0, light, brightness; e<lights.length; e++)
         {
            ...
            if (light instanceof DistantLight)
            {
               ...
               var dotVP = GlMatrix.vec3.dot(normal, light.worlddirection);
            ...
            else if (light instanceof Phoria.PointLight)
            {
               ...
               var vecToLight = GlMatrix.vec3.subtract(GlMatrix.vec3.create(), position, light.worldposition),
                   distance = GlMatrix.vec3.length(vecToLight),
                   attenuation;
               GlMatrix.vec3.normalize(vecToLight, vecToLight);
               var dotVP = GlMatrix.vec3.dot(normal, GlMatrix.vec3.negate(vecToLight, vecToLight));
               ...
               {
                  var halfV = GlMatrix.vec3.add(GlMatrix.vec3.create(), vecToLight, scene._cameraPosition),
                      dotHV = GlMatrix.vec3.dot(normal, GlMatrix.vec3.normalize(halfV, halfV)),
