...
define(function(require, exports, module) {
  ...
  var Util = require('./Util');
   ...
   var Scene = function()
   {
      ...
      this.perspective = {
         // vertical field-of-view in degrees NOTE: converted to Phoria.RADIANS for GlMatrix.mat4.perspective()
         fov: 35.0,
    ...
    */
   Scene.create = function(desc)
   {
      // merge object structures to generate scene
      var s = new Scene();
      if (desc.camera) s.camera = Util.merge(s.camera, desc.camera);
      if (desc.perspective) s.perspective = Util.merge(s.perspective, desc.perspective);
      if (desc.viewport) s.viewport = Util.merge(s.viewport, desc.viewport);
    ...
    * TODO: Unfinished!
    */
   Scene.createFromJSON = function(json)
   {
      var scene = null;
   ...
   Scene.prototype = {
      ...
      modelView: function modelView()
      {
         ...
         this._cameraPosition = GlMatrix.vec4.fromValues(
            this.camera.position.x,
            this.camera.position.y,
            this.camera.position.z,
            0);
         var camera = GlMatrix.mat4.create(),
             cameraLookat = GlMatrix.vec4.fromValues(
               this.camera.lookat.x,
               this.camera.lookat.y,
               this.camera.lookat.z,
               0),
             cameraUp = GlMatrix.vec4.fromValues(
               this.camera.up.x,
               this.camera.up.y,
               this.camera.up.z,
               0);
         ...
         GlMatrix.mat4.perspective(
            perspective,
            -this.perspective.fov * Constants.RADIANS,
         ...
         var fnProcessEntities = function processEntities(entities, matParent)
         {
            for (var n=0, obj, len, isIdentity; n<entities.length; n++)
            {
               ...
               if (matParent)
               {
                  ...
                  matLocal = matLocal ? GlMatrix.mat4.multiply(GlMatrix.mat4.create(), matLocal, matParent) : matParent;
               }
               ...
               else if (obj instanceof Entity)
               {
                  ...
                  for (var v=0, verts, vec, w, avz=0; v<len; v++)
                  {
                     ...
                     verts = obj.points[v];
                     vec = GlMatrix.vec4.set(obj._worldcoords[v], verts.x, verts.y, verts.z, 1.0);
                     ...
                     if (matLocal) GlMatrix.vec4.transformMat4(obj._worldcoords[v], vec, matLocal);
                     
                     // multiply by camera matrix to generate camera space coords
                     GlMatrix.vec4.transformMat4(obj._cameracoords[v], obj._worldcoords[v], camera);
                     
                     // multiply by perspective matrix to generate perspective and clip coordinates
                     GlMatrix.vec4.transformMat4(obj._coords[v], obj._cameracoords[v], perspective);
                  ...
                  if (objClip !== len)
                  {
                     ...
                     switch (obj.style.geometrysortmode)
                     {
                        ...
                        case "sorted":
                        {
                               ...
                               obj.style.drawmode === "solid" || obj.style.shademode === "lightsource")
                           {
                              ...
                              {
                                 case "solid":
                                    Util.sortPolygons(obj.polygons, obj._cameracoords);
                                    break;
                                 case "wireframe":
                                    Util.sortEdges(obj.edges, obj._cameracoords);
                                    break;
                                 case "point":
                                    Util.sortPoints(obj._coords, obj._worldcoords);
                                    break;
                              }
                           }
                           break;
                     ...
                     if (obj.style.drawmode === "solid" && obj.polygons.length !== 0)
                     {
                        ...
                        var matNormals = GlMatrix.mat4.invert(GlMatrix.mat4.create(), matLocal ? matLocal : GlMatrix.mat4.create());
                        GlMatrix.mat4.transpose(matNormals, matNormals);
                        ...
                        switch (obj.style.shademode)
                        {
                           case "lightsource":
                           {
                              ...
                              {
                                 if (!obj.polygons[i]._worldnormal) obj.polygons[i]._worldnormal = GlMatrix.vec4.create();
                                 ...
                                 GlMatrix.vec3.transformMat4(wnormal, normal, matNormals);
                                 GlMatrix.vec3.normalize(wnormal, wnormal);
                              }
                              break;
                           }
                           /*
                           case "gouraud":
                           {
                              ...
                              for (var i=0, normal, wnormal; i<len; i++)
                              {
                                 ...
                                 GlMatrix.vec4.transformMat4(wnormal, normal, matNormals);
                                 GlMatrix.vec4.normalize(wnormal, wnormal);
                              }
                              break;
                           }
                           */
